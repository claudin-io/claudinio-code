...
colors:
  ink-black: "#090910"
  ink-black-alt: "#0e0f16"
  fire-orange: "#5c60e6"
  cream: "#edeef5"
  cream-muted: "#b5b7c1"
  cream-hint: "#8c8e9c"
  border-dark: "#272833"
  ink-on-orange-muted: "rgba(9,9,16,0.78)"
  ink-on-orange-hint: "rgba(9,9,16,0.60)"
  ink-on-orange-faint: "rgba(9,9,16,0.45)"
  ink-on-orange-border: "rgba(9,9,16,0.24)"
...
### Hand corrections applied after the automatic remix

The role-inference repainted the system's single most important key wrongly; the
frontmatter above is the corrected, authoritative version:

- **`fire-orange` is `#5c60e6`** — the Claudinio logo indigo. In broadside this key is
  not a detail, it is *an entire register*: the declarative plane's whole ground. The
  remix had picked `#e3ae28`, the app's **warning** amber — wrong semantics, wrong brand,
  and it would have made every declarative frame read as an alert.
- **The neutral ramp is cooled to the app's own hue** — `ink-black-alt` `#0e0f16`,
  `cream-muted` `#b5b7c1`, `cream-hint` `#8c8e9c`, `border-dark` `#272833`. The remix had
  left the preset's warm greys, which clash beside the app's 280-hue surfaces.
  `cream-muted` had become the success green — a fourth hue this system forbids.

**Contrast note on the indigo register.** Near-black on `#5c60e6` is ~4.3:1 — fine for the
display ramp (AA-large) but NOT for body copy. On the indigo plane: display and headline
type in `ink-black`, anything at body size or smaller in `cream`. The `check` contrast
pass is the arbiter.
...
@font-face{font-family:"JetBrains Mono";font-weight:500;font-style:normal;font-display:block;src:url("assets/fonts/JetBrainsMono-Medium.woff2") format("woff2");}

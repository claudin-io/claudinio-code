...
## Implementation Log — 2026-08-08 09:14
**Summary:** Fix ProseContent mermaid tests by stubbing jsdom's broken localStorage (same pattern as App.test.ts)
**Changed files:** M docs/ARCHITECTURE.md, M src-tauri/Cargo.lock, M src-tauri/Cargo.toml, M src-tauri/src/agent/mcp.rs, M src-tauri/src/agent/permissions.rs, M src-tauri/src/agent/persist.rs, M src-tauri/src/agent/provider.rs, M src-tauri/src/agent/provider/openai.rs, M src-tauri/src/agent/session.rs, M src-tauri/src/agent/tools/bash.rs, M src-tauri/src/agent/tools/finalize_plan.rs, M src-tauri/src/agent/tools/mod.rs, M src-tauri/src/agent/tools/tasks.rs, M src-tauri/src/agent/tools/write_plan.rs, M src-tauri/src/agent/transition.rs, M src-tauri/src/code_intel/embeddings.rs, M src-tauri/src/commands/agent.rs, M src-tauri/src/commands/code_intel.rs, M src-tauri/src/commands/mod.rs, M src-tauri/src/lib.rs, M src-tauri/src/net_activity.rs, M src-tauri/src/state.rs, M src/components/ChatPanel.tsx, M src/components/Icon.tsx, M src/components/ProseContent.test.tsx, M src/components/SettingsPanel.tsx, M src/components/tool-renderers/ToolBody.test.tsx, M src/components/tool-renderers/ToolBody.tsx, M src/components/tool-renderers/toolPresentation.ts, M src/lib/ipc.ts, ?? .overclock-app/, ?? docs/assets/browser.png, ?? examples/, ?? scripts/pin_chromium.py, ?? src-tauri/examples/browser_check.rs, ?? src-tauri/src/agent/tools/browser.rs, ?? src-tauri/src/browser/, ?? src-tauri/src/commands/browser.rs, ?? src-tauri/src/download.rs, ?? src-tauri/src/imageutil.rs, ?? src/components/settings/SettingsBrowser.tsx
**Commits:** _(git unavailable or none)_
**Journal:** Fixed 2 failing JS tests in src/components/ProseContent.test.tsx. Root cause: the project's jsdom environment ships a broken localStorage (an empty Object whose getItem is undefined). ProseContent's Solid createEffect calls resolvedTheme() (src/lib/theme.ts), which lazily boots theme state via createThemeState() and synchronously reads localStorage.getItem('claudinio_theme') — that threw 'localStorage.getItem is not a function', aborting the effect before void renderMermaid(el) ran. So the mermaid hydration tests saw neither <svg> (success path) nor mermaid-error (failure path). The fix mirrors the existing pattern in src/App.test.ts: added a Map-backed Storage stub inside the test file's beforeAll, next to the existing matchMedia polyfill. No production code was changed — the component's resilience guards were already correct; the test environment was missing an API jsdom doesn't implement. Full suite: 709 JS tests pass (44 files), Rust tests pass.

**Task journal:**
- Review Claudin.io model pricing: Opened https://claudin.io and verified the pricing and model sections in the rendered page text.
- Screenshot model cards: Scrolled to #model-pricing and captured both model cards (Claudinio + Claudius) via browser screenshot, plus a viewport check.
- Fix ProseContent mermaid resilience tests: Root cause: jsdom's localStorage is a broken empty Object without getItem; theme.ts's createThemeState() reads localStorage.getItem synchronously when resolvedTheme() is first called from ProseContent's effect, throwing and aborting the effect before renderMermaid runs.; Fix: added a Map-backed localStorage stub (same pattern as App.test.ts) inside the existing beforeAll in ProseContent.test.tsx, next to the matchMedia polyfill.; Verification: npx vitest run src/components/ProseContent.test.tsx → 5/5 passed; full suite → 709 passed (44 files).

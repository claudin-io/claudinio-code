...
## Implementation Log — 2026-08-03 10:50
**Summary:** Fix silent turn end on promised ask_user question (judge prompt + structural backstop) and persist rejected user input to the JSONL
**Changed files:** A	docs/plans/2026-08-03_html-file-outline.md, M	src-tauri/src/code_intel/parser.rs
**Commits:** 0c88881 feat(code-intel): add HTML file outline support, f62fde1 docs(plan): html-file-outline
**Journal:** ## Fix: silent end_turn on promised ask_user question + lost rejected input

**The two bugs (both confirmed with evidence from the real Bomberman session a12edec2):**

1. **The model promised a user question but the run ended silently.** The final assistant text was "Confirmed: the chain-reacted bombs are filtered out... Fix: snapshot before filter. Now for danger memory — the core design question:" — a promise to ask a design question, with NO tool call. The terminal-turn completion judge (an LLM asked to reply CONTINUE/DONE) judged it DONE, so the run ended without ever calling ask_user and the user never saw options. The judge prompt had no explicit rule that the model can ONLY ask the user via the ask_user tool, never in prose.

2. **Rejected user input never reached the JSONL.** reject_non_english(user_message)? ran BEFORE store.try_append(&SessionRecord::User{..}), so the user's "Então..." (rejected by the English-only guard on the 'ã') vanished from the session file entirely.

**Why the judge failed:** the judge is the single gate between a dangling promise and a finish; it was told to look for "announced a next step" but had no concrete rule for the ask_user channel. LLM judges are fallible — so we added a structural backstop rather than trusting the prompt alone.

**The fix (3 parts, minimal):**
- A: COMPLETION_JUDGE_SYSTEM prompt now explicitly states the ask_user-only rule + trailing-question signals → MUST CONTINUE.
- B: should_nudge_terminal() — a pure, language-agnostic punctuation check (ends with ':' | '?' | '…' | '...') ORed into the nudge decision, so even if the judge misfires, a dangling question can't end the run. Bounded by the existing unfinished_streak < 2 cap.
- C: new SessionRecord::Rejected { text, reason, ts } persisted before the reject error is returned; the User append stays in place for accepted input. Compiler forced one extra exhaustive-match arm in list_sessions.

**Gotchas:** the SessionRecord enum is serde tag="kind" — new variant serializes as "rejected"; load_records skips undecodable lines and chatRecords.ts ignores unknown kinds, so the new record is backward/forward safe. The only compile hiccup was the non-exhaustive `match rec` in list_sessions (needed `| SessionRecord::Rejected { ts, .. }`).

**Verification:** cargo check clean; cargo test --lib 310 passed / 0 failed / 3 ignored (was 307). New tests: structural_backstop_nudges_dangling_questions (asserts the exact real case "Now for danger memory — the core design question:" → nudge), structural_backstop_spares_real_final_replies, rejected_record_serialization (round-trip kind/text/reason).

**Task journal:**
- Trace incomplete ask_user turn termination: ROOT CAUSE 1 (promise vs ask_user): The terminal-turn judge (judge_terminal_turn session.rs:1123 → classify_turn_completion provider.rs:754, prompt COMPLETION_JUDGE_SYSTEM provider.rs:738) is an LLM classifier that must answer exactly CONTINUE/DONE. It is asked only 'is this turn complete?' and its prompt had no explicit rule that a question to the user MUST be delivered via the ask_user tool. On the real Bomberman session the final text 'Now for danger memory — the core design question:' was judged DONE (JSONL continuation_judge verdict=done), so the run ended without any ask_user tool call and the user never got options.; ROOT CAUSE 2 (input not persisted): reject_non_english(user_message)? at session.rs:1457 runs BEFORE store.try_append(&SessionRecord::User {..}) and before push_user_blocks, so a rejected message (the 'Então...') was NEVER written to the JSONL. Evidence: the Bomberman JSONL ends with the error record and no User record for the rejected message.; FRONTEND TOLERANCE: chatRecords.ts only handles known kinds with silent else; load_records (persist.rs:349) skips undecodable lines via `if let Ok`. A new record kind is safe on both sides.; ask_user IS a tool: run_tool session.rs:2619 dispatches ask_user → ask_user() (session.rs:3402) which sends AgentEvent::AskUser and awaits AnswerMap; frontend renders options (ToolBody.tsx AskUserBody). A turn with an ask_user tool_use is NOT terminal — the loop never reaches the judge. So the fix is: (A) strengthen judge prompt, (B) structural punctuation backstop, (C) persist rejected input.
- Implement guard against incomplete promised questions: FIX A: COMPLETION_JUDGE_SYSTEM (provider.rs:738) reworked: explicit rule 'The assistant CAN ONLY ask the user a question through the ask_user tool, never in prose' + trailing question/dangling '…:' → verdict MUST be CONTINUE. One-sentinel language-agnostic format kept.; FIX B: new pure helper should_nudge_terminal (session.rs ~1134): returns true if text trim_end ends with ':' | '?' | '…' | '...'. Wired as `let will_nudge = verdict == TurnVerdict::Continue || should_nudge_terminal(&last_text);` at session.rs ~2076, inside the existing `unfinished_streak < 2` bound; streak logic, nudge message, golden handling untouched.; FIX C: new SessionRecord::Rejected { text, reason, ts } (persist.rs, serde rename 'rejected') right after User. In run_workflow_with_profile (~1468) the bare `reject_non_english(&user_message)?` became an if-let that appends the Rejected record + invalidate_cache before returning the error. User append stays where it was. Also added `| SessionRecord::Rejected { ts, .. }` arm to the exhaustive list_sessions match (compiler-required).; New tests: structural_backstop_nudges_dangling_questions, structural_backstop_spares_real_final_replies (session.rs), rejected_record_serialization (persist.rs) — JSON contains kind=rejected + text/reason, round-trips.
- Verify question flow and regressions: cargo check (full): Finished dev profile, no warnings.; cargo test --lib: 310 passed, 0 failed, 3 ignored (was 307 passed; +3 new tests).; Focused: cargo test --lib backstop → 2 passed; cargo test --lib rejected → 5 passed (incl. the 3 pre-existing lld_gate 'rejected' tests + new rejected_record_serialization... verified suite total).; git status: only src-tauri/src/agent/{persist,provider,session}.rs modified (+94/-9), examples/ untracked pre-existing. No other file touched.


## Implementation Log — 2026-08-03 11:32
**Summary:** Committed the ask-user/rejected-input fixes and merged feat/chain-cumulative-cost into main (updated to origin/main first).
**Changed files:** A	docs/plans/2026-08-03_html-file-outline.md, M	src-tauri/src/agent/persist.rs, M	src-tauri/src/agent/provider.rs, M	src-tauri/src/agent/session.rs, M	src-tauri/src/code_intel/parser.rs
**Commits:** 6ab3711 Merge branch 'feat/chain-cumulative-cost' into main, 7b567f9 fix(agent): preserve rejected input and require ask user, 0c88881 feat(code-intel): add HTML file outline support, f62fde1 docs(plan): html-file-outline, 90bc8b8 feat(agent): carry cumulative cost across session handoffs (#51)
**Journal:** Merge into main completed. The user-requested operation (commit all intended fixes, then merge the feature branch into main) is done and verified.

Key findings from the merge:
- The feature branch (7 commits: 3 plan docs + cumulative-cost twin + code-intel + the new ask-user fix) was NOT a descendant of updated main, so a fast-forward merge was impossible — a normal merge commit was created instead.
- The divergence was content-duplication, not a real conflict: main's PR #51 (90bc8b8) and the feature branch's local twin (c5efce8) carry IDENTICAL trees (both 7800da41...), so the ort strategy auto-merged persist.rs with no conflict markers and no manual resolution needed.
- Local main was 3 commits behind origin/main; fast-forwarded cleanly (830c8be..90bc8b8) before the merge.
- The new commit 7b567f9 "fix(agent): preserve rejected input and require ask user" contains exactly the 4 intended files (docs plan + persist/provider/session.rs), 125 insertions, 10 deletions; examples/ was correctly left untracked.

Verification after merge: cargo check finished with no warnings; cargo test --lib 310 passed, 0 failed, 3 ignored; working tree clean except the pre-existing untracked examples/ (6 files, all dated Jul 28, untouched).

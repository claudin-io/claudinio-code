...
- **UI strings** must go through the locale files in `src/lib/locales/` — never
  hardcode user-facing text. `en-US.ts` is the source of truth; the other 17 are
  generated by `scripts/translate_locales.py`.
...
- `agent/permissions.rs` — anything that widens the bash allowlist or narrows
  the denylist.
- `agent/tools/mod.rs` — the `validate_path` workspace guard.
